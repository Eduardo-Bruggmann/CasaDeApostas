...
document.getElementById("saldo").innerText = `R$ ${saldo.toLocaleString(
  "pt-BR"
)}`;
...
fetch("times.json")
  ...
  .then((data) => {
    ...
    categoriaSelect.addEventListener("change", () => {
      const categoriaSelecionada = data.categorias.find(
        (cat) => cat.nome === categoriaSelect.value
      );
      ...
      time1Select.innerHTML =
        '<option value="" disabled selected>Selecione o Time 1</option>';
      time2Select.innerHTML =
        '<option value="" disabled selected>Selecione o Time 2</option>';
      timeApostaSelect.innerHTML =
        '<option value="" disabled selected>Selecione o Time da Aposta</option>';
      ...
      function habilitarAposta() {
        if (
          time1Select.value &&
          time2Select.value &&
          time1Select.value !== time2Select.value
        ) {
